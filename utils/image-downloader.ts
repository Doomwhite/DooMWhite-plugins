...
class ImageDownloader {
	...
	async downloadImage(url: string, finalPath: string): Promise<string> {
		...
		try {
			...
			const finalFilePath = await this.moveToFinalPath(
				finalPath,
				finalFileName,
			);
	...
	private async downloadToTempFile(url: string): Promise<string> {
		console.info("Downloading image from:", url);

		const response = await axios.get(
			"http://localhost:8181/proxy?url=" + url,
			{
		...
		fs.writeFileSync(this.tempPath, Buffer.from(response.data), {
			encoding: "binary",
		});
		console.info("Image saved to temp path:", this.tempPath);
